...
public class ProfileActivity extends AppCompatActivity {
    ...
    // Skill Scores
    private TextView tvScoreWriting;
    private TextView tvScoreReading;
    private TextView tvScoreListening;
    private TextView tvScoreSpeaking;
    ...
    private void initializeViews() {
        ...
        tvScoreReading = findViewById(R.id.tvScoreReading);
        tvScoreListening = findViewById(R.id.tvScoreListening);
        tvScoreSpeaking = findViewById(R.id.tvScoreSpeaking);
    ...
    private void enableLoginRequiredFeatures() {
        ...
        btnLogout.setBackgroundTintList(
                android.content.res.ColorStateList.valueOf(getResources().getColor(android.R.color.holo_red_light)));
        btnLogout.setOnClickListener(v -> {
        ...
        btnChangeAvatar
                .setOnClickListener(v -> Toast.makeText(this, "Tính năng đang phát triển", Toast.LENGTH_SHORT).show());
        ...
        db.collection("user_achievements").document(userId)
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        java.util.List<String> unlockedAchievements = (java.util.List<String>) documentSnapshot
                                .get("unlocked_achievements");
    ...
    private String getAchievementIcon(String title) {
        if (title.contains("Người mới"))
            return "🌱";
        if (title.contains("Học viên"))
            return "🌿";
        if (title.contains("Chuyên gia") && !title.contains("XP"))
            return "🌳";
        if (title.contains("Bậc thầy") && !title.contains("XP"))
            return "🏆";
        if (title.contains("Nhất quán"))
            return "🔥";
        if (title.contains("Kiên trì"))
            return "⚡";
        if (title.contains("Huyền thoại"))
            return "💎";
        if (title.contains("Thu thập XP"))
            return "⭐";
        if (title.contains("Chuyên gia XP"))
            return "🌟";
        if (title.contains("Bậc thầy XP"))
            return "✨";
        if (title.contains("đọc"))
            return "📖";
        if (title.contains("thính"))
            return "👂";
        if (title.contains("văn"))
            return "✍️";
        if (title.contains("giả"))
            return "🗣️";
        return "🏆";
    }
    ...
    private void displayUserProgress(DocumentSnapshot document) {
        if (document.exists()) {
            ...
            Map<String, Double> skillScores = (Map<String, Double>) document.get("skill_scores");
            if (skillScores != null) {
                displaySkillScore(tvScoreWriting, skillScores.get(SkillManager.SKILL_WRITING));
                displaySkillScore(tvScoreReading, skillScores.get(SkillManager.SKILL_READING));
                displaySkillScore(tvScoreListening, skillScores.get(SkillManager.SKILL_LISTENING));
                displaySkillScore(tvScoreSpeaking, skillScores.get(SkillManager.SKILL_SPEAKING));
            }
    ...
    private void displaySkillScore(TextView tv, Double score) {
        if (tv == null)
            return;
        ...
        if (val >= 8.0) {
            tv.setTextColor(getResources().getColor(R.color.success)); // Green
        } else if (val >= 5.0) {
            tv.setTextColor(getResources().getColor(R.color.primary)); // Blue
        } else {
            tv.setTextColor(getResources().getColor(R.color.error)); // Red
        }
    }
